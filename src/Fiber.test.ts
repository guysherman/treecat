...
describe('Fiber', () => {
  ...
  it('Box with multiple boxes', () => {
    ...
    // now have a dom. We need this to be the case for any host
    // component, because when we come to commit the work, we run
    // up the tree skipping fibers without dom elements, so that we
    // can effectively compact the function components out of the tree
  ...
  it('1 box(foo,bar,baz) -> 1 box(foo, bar) => 1 update(update, update, delete)', () => {
    const root = {
      dom: rootScreen,
      effects: [],
      effectCleanups: [],
  ...
  it('1 box(foo,bar,baz) -> 1 box(foo, baz) => 1 update(update, placement, delete, delete)', () => {
    const root = {
      dom: rootScreen,
      effects: [],
      effectCleanups: [],
